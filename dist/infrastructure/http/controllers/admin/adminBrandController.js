...
exports.listPendingBrandsController = listPendingBrandsController;
exports.approveBrandController = approveBrandController;
exports.rejectBrandController = rejectBrandController;
const prismaClient_1 = require("../../../prisma/prismaClient");
const resendProvider_1 = require("../../../email/resendProvider");
/**
 * List all brands for admin (with pagination and filtering)
 */
async function listAllBrandsController(req, res) {
    try {
        ...
        const [brands, total] = await Promise.all([
            ...
            prismaClient_1.prisma.brand.count({ where })
        ]);
        return res.json({
            items: brands,
            total,
 ...
 */
async function listPendingBrandsController(req, res) {
    try {
 ...
 */
async function approveBrandController(req, res) {
    try {
 ...
 */
async function rejectBrandController(req, res) {
    try {
        const brandId = Number(req.params.brandId);
        const adminId = req.auth?.userId;
...
// EMAIL TEMPLATES
// =====================
function getApprovalEmailTemplate(userName, brandName) {
    return `
...
function getRejectionEmailTemplate(userName, brandName, reason) {
    return `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
      <p style="font-size: 16px; color: #333;">Bonjour <strong>${userName}</strong>,</p>
