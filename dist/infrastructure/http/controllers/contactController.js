...
const sendContactEmail = async (req, res, next) => {
    try {
        ...
        if (!name || !email || !subject || !message) {
            throw new AppError_1.AppError("Tous les champs sont requis", 400);
        }
        ...
        const emailSent = await (0, resendProvider_1.sendEmail)({
            to: supportEmail,
        ...
        if (!emailSent) {
            throw new AppError_1.AppError("Erreur lors de l'envoi de l'email", 500);
        }
        res.status(200).json({
            status: 'success',
