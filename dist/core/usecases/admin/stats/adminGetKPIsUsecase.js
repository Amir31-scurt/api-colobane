...
exports.adminGetKPIsUsecase = adminGetKPIsUsecase;
...
async function adminGetKPIsUsecase() {
    ...
    const [totalOrders, totalRevenueData, totalUsers] = await Promise.all([
        ...
        prismaClient_1.prisma.user.count({ where: { role: "CUSTOMER" } }),
    ]);
    ...
    const topProductsDetails = await prismaClient_1.prisma.product.findMany({
    ...
    const [newProductsCount, prevNewProductsCount] = await Promise.all([
        prismaClient_1.prisma.product.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
        prismaClient_1.prisma.product.count({ where: { createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } })
    ]);
    const [newSellersCount, prevNewSellersCount] = await Promise.all([
        prismaClient_1.prisma.user.count({ where: { role: "SELLER", createdAt: { gte: thirtyDaysAgo } } }),
        prismaClient_1.prisma.user.count({ where: { role: "SELLER", createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } })
    ]);
    ...
    return {
        ...
        products: { value: await prismaClient_1.prisma.product.count(), trend: trends.products },
        sellers: { value: await prismaClient_1.prisma.user.count({ where: { role: "SELLER" } }), trend: trends.sellers },
        // Legacy fields
        totalRevenue,
        totalOrders,
        totalCustomers: totalUsers,
        totalProducts: await prismaClient_1.prisma.product.count(),
        totalSellers: await prismaClient_1.prisma.user.count({ where: { role: "SELLER" } }),
        topProducts,
